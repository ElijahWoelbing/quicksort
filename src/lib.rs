...
fn swap(arr: &mut Vec<i32>, pos_1: i32, pos_2: i32) {
...
mod tests {
    ...
    #[test]
    fn quicksort_test() {
        let mut numbers = vec![4, 3, 7, 5, 6, 9, 1, 8];
        let last = numbers.len() - 1;
        quicksort(&mut numbers, 0, last as i32);
        let sorted_numbers: Vec<i32> = vec![1, 3, 4, 5, 6, 7, 8, 9];
        assert_eq!(sorted_numbers, numbers);
    }
    #[test]
    fn swap_test() {
        let mut numbers = vec![4, 3, 6];
        swap(&mut numbers, 0, 2);
        let swaped_numbers = vec![6, 3, 4];
        assert_eq!(swaped_numbers, numbers);
    }
    #[test]
    fn partition_test() {
        let mut numbers = vec![4, 2, 6, 3, 8, 5];
        let last = numbers.len() - 1;
        partition(&mut numbers, 0, last as i32);
        let partitioned_numbers = vec![4, 2, 3, 5, 8, 6];
